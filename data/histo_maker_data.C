...
#include "TObject.h"
#include <iostream>
//g++ -g  -Wno-deprecated histo_maker_data.C -o findevn.exe  -I$ROOTSYS/include -L$ROOTSYS/lib `root-config --cflags` `root-config --libs`
...
int main(int argc, char *argv[]){
  cout<<"Program started"<<endl;
        ...
        if(inputFile=="FILELIST/JetHT_Run2017B_EOY_16_04_2021.log"){
                sprintf(fOut,"HIST/JetHT_Run2017B_EOY_16_04_2021_%s_%s.root",argv[1],argv[2]);
        }
        else if(inputFile=="FILELIST/JetHT_Run2017C_EOY_16_04_2021.log"){
                sprintf(fOut,"HIST/JetHT_Run2017C_EOY_16_04_2021_%s_%s.root",argv[1],argv[2]);
        }
        else if(inputFile=="FILELIST/JetHT_Run2017D_EOY_16_04_2021.log"){
                sprintf(fOut,"HIST/JetHT_Run2017D_EOY_16_04_2021_%s_%s.root",argv[1],argv[2]);
        }
        else if(inputFile=="FILELIST/JetHT_Run2017E_EOY_16_04_2021.log"){
                sprintf(fOut,"HIST/JetHT_Run2017E_EOY_16_04_2021_%s_%s.root",argv[1],argv[2]);
        }
        else if(inputFile=="FILELIST/JetHT_Run2017F_EOY_16_04_2021.log"){
                sprintf(fOut,"HIST/JetHT_Run2017F_EOY_16_04_2021_%s_%s.root",argv[1],argv[2]);
        }
  ...
  //*************************************************************************************************************************//
   ...
   Double_t met, pumet, metphi, pumetphi, gen_wt;
   Int_t nv;
   Bool_t          HLT;
   ...
   met_tree->SetBranchAddress("_genWeight",&gen_wt);
   //MET Variables
   ...
   for(int iEntry = 0; iEntry < nEntries; ++iEntry)
    {
       ...
       if(hlt_1 || hlt_2 || hlt_3 || hlt_4) 
         {
           h_evt->Fill(2);
           float wt = 1.0; //gen_wt / h_wt->GetBinContent(2);
           if (met < 300) continue;
           h_evt->Fill(3);
